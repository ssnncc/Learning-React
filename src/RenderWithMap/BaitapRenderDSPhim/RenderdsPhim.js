...
import style from './BaitapDSphim.module.css'//Nhúng file css 
...
export default class RenderdsPhim extends Component {

    renderMovieWithMap=()=>{
        
        const MapPhim = mangPhim.map((phim,index)=>{
               return <div className="col-4 pt-3" key={index} >
               <div className="card">
              {/* Cách 1 : Tạo thẻ css và nhúng như bình thường  */}
              {/* Cách 2 */}
              {/* <img className={`card-img-top ${style.hinhAnhPhim}`} src={phim.hinhAnh} alt="134" /> */}
              {/* Hoạc có thể truy xuất như vậy */}
              {/* <img className={`card-img-top ${style['hinhAnhPhim']}`} src={phim.hinhAnh} alt="134" /> */}
              {/* Cách 3  */}
              <img style={{width:`auto`, height: `250px`}} className="card-img-top" src={phim.hinhAnh} alt="134" />
               <div className="card-body">
        <h4 className="card-title" style={{height: '60px'}}>{phim.tenPhim.length >50 ? <span>{phim.tenPhim.substr(0,50)}...</span> :<span>{phim.tenPhim}</span>}</h4>
    ...
    render() {
        return (
            <div className="container">
                <div className="text-center display-5  text-danger p-3">DANH SÁCH PHIM</div>
                <div className="row">
                {this.renderMovieWithMap()}
                </div>
